  ...
  * target tokens are presented as (x[t-1],x[t]) pairs
  ...
  (except each word will be represented by a one-hot vector, and with
    ...
    type S2SData; bgen1; bgen2;
      function S2SData(sgen1, sgen2; batchsize=128, atype=Array{Float32}, dense=true, o...)
        new(S2SBatch(sgen1, batchsize, atype, dense),
        S2SBatch(sgen2, batchsize, atype, dense))
      end
    end
    ...
    type S2SBatch; sgen; state; batch; x; y; mask; done;
      function S2SBatch(sgen, batchsize, atype, dense)
        ftype=Float32
        #ftype = eltype(atype)
        dense = true # dense!
        #println(eltype(atype))
        if dense
          x = zeros(ftype, batchsize, eos(sgen))
          y = zeros(ftype, batchsize, eos(sgen))
        else
          # sparse matrices in Knet8 are deprecated
          # ignore this for now
          x = sponehot(ftype, batchsize, eos(sgen))
          y = sponehot(ftype, batchsize, eos(sgen))
        end
  ...
  #  setrow!(x::SparseMatrixCSC,i,j)=(i>0 ? (x.rowval[j] = i; x.nzval[j] = 1) : (x.rowval[j]=1; x.nzval[j]=0); x)
    setrow!(x::Array,i,j)=(x[i,:]=0; j>0 && (x[i,j]=1); x)
    setrow!(x::KnetArray,i,j)=(x[i,:]=0; j>0 && (x[i,j]=1); x)

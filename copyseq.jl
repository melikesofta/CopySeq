...
function main(args=ARGS)
  ...
  if o[:savefile] != nothing
    ...
    model = map(p->convert(Array{Float32}, p), model)
    save(o[:savefile], "model", model, "data", data)
  end
  if o[:copy] != nothing
    state = initstate(o[:atype], o[:hidden], 1)
    copydata = Data(o[:copy]; batchsize=1, word_to_index=dict, serve_type="onehot")
    copysequence(model, state, vocab, copydata, o)
  end
end
...
function copysequence(params, state, vocab, copydata, o)
  for batch in copydata
    ...
    for word in batch
      state, ypred = encdec(params, input, state, decoding) #predict
      input = ypred
      #println(indmax(convert(Array{Float32}, ypred)))
      #println(convert(Array{Float32}, ypred))
      #print(length(copydata.index_to_word))
      print(copydata.index_to_word[indmax(convert(Array{Float32}, ypred))])
      print(" ")
    end
